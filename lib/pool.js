...
function cryptoNightFast(buf) {
    return cryptoNight(Buffer.concat([Buffer.from([buf.length]), buf]), true);
}
...
(function init() {
    jobRefresh(true, function(sucessful) {
        ...
        startPoolServerTcp((successful) => {});
        startPoolServerWeb((successful) => {});
    });
})();
...
function startPoolServerWeb(callback) {

  app = express();
  app.use(express.static(config.web.folder));

  server = http.createServer(app);

  server.listen(config.web.port, function (error, result) {
    if (error) {
      log('error', logSystem, 'Could not start WEB server listening on port %d, error: $j', [config.web.port, error]);
      process.exit(1);
    }
    log('info', logSystem, 'Started WEB server listening on port %d', [config.web.port]);
  });

  async.each(config.poolServer.websockets, function(wsData, cback) {
      const handleMessage = function(socket, jsonData, pushMessage) {
          if (!jsonData.id) {
              log('warn', logSystem, 'Miner WS RPC request missing RPC id');
              return;
          }
          else if (!jsonData.method) {
              log('warn', logSystem, 'Miner WS RPC request missing RPC method');
              return;
          }
          else if (!jsonData.params) {
              log('warn', logSystem, 'Miner WS RPC request missing RPC params');
              return;
          }
          ...
          const sendReply = function(error, result) {
              ...
              socket.send(sendData, (e) => {
                if (e) {
                    log('warn', logSystem, 'WS Socket error from %s %j', [socket.ip, e]);
                }
              });
          };

          handleMinerMethod(jsonData.method, jsonData.params, socket.remoteAddress, wsData, sendReply, pushMessage);
      };

      const wss = new WebSocket.Server({
        server,
        path: wsData.path,
        maxPayload: wsData.maxPayload || 2048,
      });

      wss.on('connection', (socket, req) => {
          const ip = socket.ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress;

          const pushMessage = function(method, params) {
              ...
              socket.send(sendData, (e) => {
                if (e) {
                   log('warn', logSystem, 'WS Socket error from %s %j', [ip, e]);
                }
              });
          };
          ...
          socket.on('message', function incomming(message) {
            ...
            try {
                ...
                log('warn', logSystem, 'WS Malformed message from %s: %s', [ip, e]);
                socket.close();
            }
